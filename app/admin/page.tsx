    ...
    <div className="space-y-12">
      ...
      <section className="border-4 border-govuk-black p-6">
        ...
        <form onSubmit={handleVerify} className="space-y-4 max-w-[400px]">
          <div>
            <Label htmlFor="shareCode" className="font-bold">Enter 9-character share code</Label>
            <Input
              id="shareCode"
              value={verifyCode}
        ...
        {verifyResult && (
          <div className={`mt-6 p-4 border-l-8 ${verifyResult.valid ? 'bg-green-50 border-green-600' : 'bg-red-50 border-red-600'}`}>
            <div className="flex items-center gap-2 mb-2">
              {verifyResult.valid ? <CheckCircle className="text-green-600" /> : <XCircle className="text-red-600" />}
              <span className="font-bold text-xl">{verifyResult.valid ? 'Valid Code' : 'Invalid Code'}</span>
            </div>
            {verifyResult.valid ? (
              <div>
      ...
      <section>
        ...
        <form onSubmit={handleRegister} className="max-w-[600px] space-y-6 bg-govuk-grey-3 p-8 border border-govuk-grey-2">
          {regError && (
            <div className="bg-red-50 border-2 border-red-600 p-4 mb-4 text-red-700 font-bold">
              {regError}
            </div>
          )}
          {regSuccess && (
          ...
          <div>
            <Label htmlFor="fullName" className="font-bold mb-1 block">Full Name</Label>
            <Input
              id="fullName"
          ...
          <div>
            <Label htmlFor="dob" className="font-bold mb-1 block">Date of Birth</Label>
            <Input
              id="dob"
              type="date"
          ...
          <div>
            ...
            <Label htmlFor="docNumber" className="font-bold mb-1 block">Document Number</Label>
            <Input
              id="docNumber"
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {isLoadingUsers ? (
                ...
                <tr>
                  <td colSpan={4} className="py-8 text-center text-govuk-grey-1">No users found</td>
                </tr>
              ) : (
                users.map((user) => (
                  <tr key={user.id} className="border-b border-govuk-grey-2">
                    ...
                    <td className="py-4 pr-4">
                      <span className={`px-2 py-1 text-xs font-bold rounded ${user.account_status === 'ACTIVE' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {user.account_status}
                      </span>
                    </td>
